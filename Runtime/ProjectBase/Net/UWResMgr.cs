...
namespace Momos.Core.Net {
    public class UWResMgr : SingletonAutoMono<UWResMgr>, ISingletonMono {
        ...
        public void LoadRes<T>(string path, UnityAction<T> callBack, UnityAction failCallBack) where T : class {
            StartCoroutine(IE_LoadRes<T>(path, callBack, failCallBack));
        }

        private IEnumerator IE_LoadRes<T>(string path, UnityAction<T> callBack, UnityAction failCallBack) where T : class {
            //string
            //byte[]
            //Texture
            //AssetBundle
